...
When you genuinely need a floating window, `Super + T` toggles the active one out of the tiling (and back). But give the tiling a real chance first — it's the heart of the whole thing. [Navigation](../navigation/) walks you through it.
...
Windows folks: your Win + V clipboard history lives on `Super + Ctrl + V`, and it holds images as well as text. See [unified clipboard & history](../unified-clipboard-history/).
...
| AirDrop | LocalSend, via `Super + Ctrl + S` — see [GUIs](../guis/) |
| Cmd + Shift + 4 / Win + Shift + S | `Print Screen` — see [screenshots & recording](../screenshots-recording/) |
| Notification Center | Notification history on `Super + Shift + Alt + ,` |
| Time Machine (for the system) | Automatic [system snapshots](../system-snapshots/) on every update |
| App Store / downloading an installer | _Install_ in the menu, or `omarchy pkg add` — see [other packages](../other-packages/) |
| System Settings / Control Panel | _Setup_ in the menu, which edits plain config files — see [dotfiles](../dotfiles/) |
...
Updates come through one command — _Update > Omarchy_ — that updates Omarchy itself and every package on the system, taking a snapshot first. No per-app updaters nagging you at random. See [updates](../updates/).
...
Omarchy runs well on Intel Macs — see [Mac support](../mac-support/). And the keyboard is kind to you: Omarchy doesn't remap anything, and Linux treats the Command key as Super, so Super sits right where Cmd always was. Your thumb won't notice the move.

### Give it two weeks

The instincts transfer faster than you'd think. Skim the [hotkeys](../hotkeys/) chapter once, and whenever you blank on a binding, hit `Super + K` — it shows you all of them. That's the only hotkey you actually have to memorize.
